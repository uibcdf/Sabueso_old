...
def is_strand_dict(item):

    output = False

    if type(item) is dict:
        if set(strand_dict)==set(item):
            output = True

    return output

class StrandCard(Card):

    def __init__(self, item=None):

        super().__init__()

        self.card_type = 'strand'

        if is_strand_dict(item):
    ...
    def __repr__(self):
        ...
        return f'<StrandCard: {self.description} {self.begin}-{self.end}>'
